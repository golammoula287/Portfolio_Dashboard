...
import { resetPassword, clearAllForgotPasswordError } from '@/store/slices/forgotResetPasswordSlice'; // Adjust imports as needed
import { toast } from 'react-toastify';
import { getUser } from '@/store/slices/userSlice';

const ResetPassword = () => {
  const { token } = useParams(); // Ensure useParams is imported
  ...
  useEffect(() => {
    if (error) {
      toast.error(error);
      dispatch(clearAllForgotPasswordError()); // Ensure this action exists
    }
    if (isAuthenticated) {
      navigateTo('/');
    }
